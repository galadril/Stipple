...
using stipple::net::sntp::kEpochOffsetSeconds;
using stipple::net::sntp::kPacketBytes;
using stipple::net::sntp::Reply;
...
STIPPLE_TEST(Sntp, RequestIsAClientPacket) {
    std::uint8_t packet[kPacketBytes];
    std::memset(packet, 0xAA, sizeof(packet));
    stipple::net::sntp::build(packet);

    STIPPLE_CHECK_EQ(static_cast<int>((packet[0] >> 3) & 0x7), 4);  // version 4
    STIPPLE_CHECK_EQ(static_cast<int>(packet[0] & 0x7), 3);         // mode client
    ...
    for (std::size_t i = 1; i < kPacketBytes; ++i) {
        STIPPLE_CHECK_EQ(static_cast<int>(packet[i]), 0);
    }
}

STIPPLE_TEST(Sntp, ReadsATimeBack) {
    std::uint8_t packet[kPacketBytes];
    ...
    Reply reply;
    STIPPLE_CHECK(stipple::net::sntp::parse(packet, sizeof(packet), reply));
    STIPPLE_CHECK_EQ(reply.unixSeconds, when);
    STIPPLE_CHECK_EQ(static_cast<int>(reply.stratum), 2);
}

STIPPLE_TEST(Sntp, RefusesAShortPacket) {
    std::uint8_t packet[kPacketBytes];
    makeReply(packet, 1790708400u);
    Reply reply;
    STIPPLE_CHECK(!stipple::net::sntp::parse(packet, kPacketBytes - 1, reply));
    STIPPLE_CHECK(!stipple::net::sntp::parse(nullptr, kPacketBytes, reply));
}

STIPPLE_TEST(Sntp, RefusesAnUnsynchronisedServer) {
    std::uint8_t packet[kPacketBytes];
...
STIPPLE_TEST(Sntp, RefusesKissOfDeath) {
    std::uint8_t packet[kPacketBytes];
...
STIPPLE_TEST(Sntp, RefusesAnImpossibleStratum) {
    std::uint8_t packet[kPacketBytes];
...
STIPPLE_TEST(Sntp, RefusesSomethingThatIsNotAServerReply) {
    std::uint8_t packet[kPacketBytes];
...
STIPPLE_TEST(Sntp, RefusesATimestampBeforeTheUnixEpoch) {
    std::uint8_t packet[kPacketBytes];
    makeReply(packet, 0);
    // A zero transmit timestamp means the server did not fill it in.
    STIPPLE_CHECK_EQ(static_cast<int>(packet[40]), 0x83);
...
STIPPLE_TEST(Sntp, AcceptsABroadcastReply) {
    std::uint8_t packet[kPacketBytes];
    makeReply(packet, 1790708400u, 3, 0, 5);
    Reply reply;
    STIPPLE_CHECK(stipple::net::sntp::parse(packet, sizeof(packet), reply));
    STIPPLE_CHECK_EQ(reply.unixSeconds, 1790708400u);
}
