...
NOTRIX_TEST(Md5, MatchesTheVectorsInRfc1321) {
    ...
    NOTRIX_CHECK_EQ(hex(""), std::string("d41d8cd98f00b204e9800998ecf8427e"));
    NOTRIX_CHECK_EQ(hex("a"), std::string("0cc175b9c0f1b6a831c399e269772661"));
    NOTRIX_CHECK_EQ(hex("abc"), std::string("900150983cd24fb0d6963f7d28e17f72"));
    NOTRIX_CHECK_EQ(hex("message digest"),
                    std::string("f96b697d7cb7938d525a2f31aaf161d0"));
    NOTRIX_CHECK_EQ(hex("abcdefghijklmnopqrstuvwxyz"),
...
NOTRIX_TEST(Md5, HandlesTheBlockBoundariesThatTripImplementationsUp) {
    ...
    for (std::size_t length : {std::size_t(54), std::size_t(55), std::size_t(56),
        ...
        NOTRIX_CHECK_EQ(incremental.finishHex(), Md5::hex(block.data(), block.size()));
    }
}

NOTRIX_TEST(Md5, FeedingItInPiecesGivesTheSameAnswer) {
    ...
    NOTRIX_CHECK_EQ(inPieces.finishHex(), oneGo.finishHex());
}

NOTRIX_TEST(Md5, HandlesBytesThatAreNotText) {
    std::uint8_t raw[256];
    for (int i = 0; i < 256; ++i) {
        raw[i] = static_cast<std::uint8_t>(i);
    }
    NOTRIX_CHECK_EQ(Md5::hex(raw, sizeof(raw)),
                    std::string("e2c865db4162bed963bfaa9ef6ac18f0"));
}

NOTRIX_TEST(Md5, ResetMakesItReusable) {
    Md5 md5;
    md5.update("abc", 3);
    NOTRIX_CHECK_EQ(md5.finishHex(), std::string("900150983cd24fb0d6963f7d28e17f72"));

    md5.reset();
    md5.update("a", 1);
    NOTRIX_CHECK_EQ(md5.finishHex(), std::string("0cc175b9c0f1b6a831c399e269772661"));
}
