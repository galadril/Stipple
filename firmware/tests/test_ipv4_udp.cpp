...
using stipple::net::ipv4::Datagram;
using stipple::net::ipv4::encapsulate;
using stipple::net::ipv4::extract;
using stipple::net::ipv4::kHeaderBytes;
using stipple::net::ipv4::kIpHeaderBytes;
...
STIPPLE_TEST(Ipv4Udp, WrapsAndUnwraps) {
    const std::vector<std::uint8_t> frame =
        wrap(ip(0, 0, 0, 0), ip(255, 255, 255, 255), kPayload, sizeof(kPayload));
    STIPPLE_CHECK_EQ(frame.size(), kHeaderBytes + sizeof(kPayload));

    Datagram out;
    STIPPLE_REQUIRE(extract(frame.data(), frame.size(), out));
    STIPPLE_CHECK_EQ(out.source, 0u);
    STIPPLE_CHECK_EQ(out.destination, ip(255, 255, 255, 255));
    STIPPLE_CHECK_EQ(int(out.sourcePort), 68);
    STIPPLE_CHECK_EQ(int(out.destinationPort), 67);
    STIPPLE_CHECK_EQ(out.payloadSize, sizeof(kPayload));
    for (std::size_t i = 0; i < sizeof(kPayload); ++i) {
        STIPPLE_CHECK_EQ(int(out.payload[i]), int(kPayload[i]));
    }
}

STIPPLE_TEST(Ipv4Udp, WritesAHeaderThatLooksRight) {
    const std::vector<std::uint8_t> frame =
        wrap(ip(192, 168, 1, 238), ip(192, 168, 1, 1), kPayload, sizeof(kPayload));

    STIPPLE_CHECK_EQ(int(frame[0]), 0x45);  // IPv4, no options
    STIPPLE_CHECK_EQ(int(frame[9]), 17);    // UDP
    STIPPLE_CHECK_EQ(int(frame[8]), 64);    // ttl

    const std::size_t declared = static_cast<std::size_t>((frame[2] << 8) | frame[3]);
    STIPPLE_CHECK_EQ(declared, frame.size());
    ...
    STIPPLE_CHECK(udpChecksum != 0);
}

STIPPLE_TEST(Ipv4Udp, ChecksumsAreRightAccordingToTheOtherDirection) {
    ...
    STIPPLE_CHECK_EQ(total & 0xFFFFu, 0xFFFFu);
}

STIPPLE_TEST(Ipv4Udp, RejectsACorruptHeader) {
    std::vector<std::uint8_t> frame =
        wrap(ip(0, 0, 0, 0), ip(255, 255, 255, 255), kPayload, sizeof(kPayload));
    frame[12] ^= 0xFFu;  // change a source octet, leave the checksum alone

    Datagram out;
    STIPPLE_CHECK_FALSE(extract(frame.data(), frame.size(), out));
}

STIPPLE_TEST(Ipv4Udp, RejectsACorruptPayload) {
    std::vector<std::uint8_t> frame =
        wrap(ip(0, 0, 0, 0), ip(255, 255, 255, 255), kPayload, sizeof(kPayload));
...
STIPPLE_TEST(Ipv4Udp, IgnoresEverythingThatIsNotUdp) {
    ...
    STIPPLE_CHECK_FALSE(extract(tcp.data(), tcp.size(), out));

    std::vector<std::uint8_t> six = frame;
    six[0] = 0x60;
    STIPPLE_CHECK_FALSE(extract(six.data(), six.size(), out));

    STIPPLE_CHECK_FALSE(extract(frame.data(), 4, out));
    STIPPLE_CHECK_FALSE(extract(nullptr, 100, out));
}

STIPPLE_TEST(Ipv4Udp, IgnoresFragments) {
    ...
    STIPPLE_CHECK_FALSE(extract(later.data(), later.size(), out));

    std::vector<std::uint8_t> more = frame;
    more[6] = 0x20;  // more-fragments
    refreshIpChecksum(more);
    STIPPLE_CHECK_FALSE(extract(more.data(), more.size(), out));
}

STIPPLE_TEST(Ipv4Udp, RefusesLengthsThatWouldReadPastTheBuffer) {
    ...
    STIPPLE_CHECK_FALSE(extract(lying.data(), lying.size(), out));

    // A UDP length longer than the IP total length says is available.
    std::vector<std::uint8_t> udpLying = frame;
    udpLying[24] = 0x0F;
    udpLying[25] = 0xFF;
    STIPPLE_CHECK_FALSE(extract(udpLying.data(), udpLying.size(), out));
    ...
    STIPPLE_CHECK_FALSE(extract(ihl.data(), ihl.size(), out));
}

STIPPLE_TEST(Ipv4Udp, AcceptsADatagramWithNoChecksum) {
...
STIPPLE_TEST(Ipv4Udp, HandlesAnOddNumberOfPayloadBytes) {
    ...
    Datagram out;
    STIPPLE_REQUIRE(extract(frame.data(), frame.size(), out));
    STIPPLE_CHECK_EQ(out.payloadSize, sizeof(odd));
    STIPPLE_CHECK_EQ(int(out.payload[2]), 0xEF);
}

STIPPLE_TEST(Ipv4Udp, RefusesToWriteIntoABufferTooSmall) {
    std::uint8_t small[16];
    STIPPLE_CHECK_EQ(encapsulate(kPayload, sizeof(kPayload), 0, 0xFFFFFFFFu, 68, 67, 1, small,
                                sizeof(small)),
                    std::size_t(0));
}
