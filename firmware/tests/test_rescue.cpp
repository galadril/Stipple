...
NOTRIX_TEST(Rescue, BothButtonsHeldLongEnoughFires) {
    Rescue rescue;
    NOTRIX_CHECK(holdFor(rescue, 1000, 1000 + Rescue::kHoldMillis + 100));
}

NOTRIX_TEST(Rescue, ItFiresExactlyOnce) {
...
NOTRIX_TEST(Rescue, OneButtonIsNotEnough) {
    ...
    NOTRIX_CHECK_FALSE(rescue.counting());
}

NOTRIX_TEST(Rescue, LettingGoEarlyStartsOverRatherThanResuming) {
    ...
    for (std::uint64_t now = 0; now < Rescue::kHoldMillis - 1000; now += 50) {
        NOTRIX_CHECK_FALSE(rescue.tick(now));
    }
...
NOTRIX_TEST(Rescue, TheOrderTheButtonsGoDownDoesNotMatter) {
...
NOTRIX_TEST(Rescue, TheCountdownIsVisibleWhileItRuns) {
    ...
    NOTRIX_CHECK(rescue.counting());
    NOTRIX_CHECK_EQ(rescue.remainingMillis(1000), Rescue::kHoldMillis);
    NOTRIX_CHECK_EQ(rescue.remainingMillis(3000), Rescue::kHoldMillis - 2000);

    // And it stops once spent, rather than reading as a fresh countdown.
    rescue.tick(1000 + Rescue::kHoldMillis);
    NOTRIX_CHECK_EQ(rescue.remainingMillis(1000 + Rescue::kHoldMillis), 0u);
}

NOTRIX_TEST(Rescue, NothingIsCountingBeforeAnythingIsHeld) {
    Rescue rescue;
    NOTRIX_CHECK_FALSE(rescue.counting());
    NOTRIX_CHECK_EQ(rescue.remainingMillis(10000), 0u);
    NOTRIX_CHECK_FALSE(rescue.tick(10000));
}

NOTRIX_TEST(Rescue, ARotaryDetentCannotBeHeld) {
    Rescue rescue;
    InputEvent tick;
...
NOTRIX_TEST(Rescue, AClockSteppingBackwardsCannotCompleteTheHoldEarly) {
    ...
    NOTRIX_CHECK_FALSE(rescue.tick(5000));  // clock jumped back
    NOTRIX_CHECK(rescue.counting());

    // The hold now runs from the corrected time.
    NOTRIX_CHECK_FALSE(rescue.tick(5000 + Rescue::kHoldMillis - 100));
    NOTRIX_CHECK(rescue.tick(5000 + Rescue::kHoldMillis));
}

NOTRIX_TEST(Rescue, ResetForgetsAHoldInProgress) {
    Rescue rescue;
    rescue.handle(press(RawInput::KeyMinus, 0));
    rescue.handle(press(RawInput::KeyPlus, 0));
    NOTRIX_CHECK(rescue.counting());
    ...
    NOTRIX_CHECK_FALSE(rescue.counting());
    NOTRIX_CHECK_FALSE(rescue.tick(Rescue::kHoldMillis * 2));
}
