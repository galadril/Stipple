...
using notrix::audio::ToneGenerator;
...
NOTRIX_TEST(Tone, SilentUntilAskedForSomething) {
    ToneGenerator tone;
    NOTRIX_CHECK_FALSE(tone.playing());

    std::int16_t samples[64];
    NOTRIX_CHECK_EQ(tone.fill(samples, 64), 0);
    for (int i = 0; i < 64; ++i) {
        NOTRIX_CHECK_EQ(static_cast<int>(samples[i]), 0);
    }
}

NOTRIX_TEST(Tone, DurationDecidesHowManySamplesArrive) {
    ToneGenerator tone(16000);
    tone.start(1000, 100);  // 100 ms at 16 kHz
    NOTRIX_CHECK_EQ(tone.remaining(), 1600);

    std::int16_t samples[1600];
    NOTRIX_CHECK_EQ(tone.fill(samples, 1600), 1600);
    NOTRIX_CHECK_FALSE(tone.playing());
}

NOTRIX_TEST(Tone, TheFrequencyAskedForIsTheFrequencyProduced) {
    ToneGenerator tone(16000);
    ...
    NOTRIX_CHECK(changes > 1900);
    NOTRIX_CHECK(changes < 2100);
}

NOTRIX_TEST(Tone, VolumeScalesTheSamples) {
    ...
    NOTRIX_CHECK(peak(100) > peak(50));
    NOTRIX_CHECK(peak(50) > peak(10));
    NOTRIX_CHECK_EQ(peak(0), 0);
}

NOTRIX_TEST(Tone, AShortToneZeroFillsTheRestOfItsFrame) {
    ...
    NOTRIX_CHECK_EQ(tone.fill(samples, 128), 16);
    for (int i = 16; i < 128; ++i) {
        NOTRIX_CHECK_EQ(static_cast<int>(samples[i]), 0);
    }
}

NOTRIX_TEST(Tone, StartingAgainReplacesRatherThanQueues) {
    // A device that queued beeps would fall behind the thing it is beeping
    // about, and the queue is unbounded by nature.
    ToneGenerator tone(16000);
    tone.start(1000, 1000);
    tone.start(1000, 10);
    NOTRIX_CHECK_EQ(tone.remaining(), 160);
}

NOTRIX_TEST(Tone, NonsenseIsRefusedRatherThanRendered) {
    ToneGenerator tone(16000);
...
NOTRIX_TEST(Tone, DurationIsBounded) {
    ...
    NOTRIX_CHECK(tone.remaining() <= 16000 * 10);
}

NOTRIX_TEST(Tone, AFrequencyAboveWhatTheRateCanCarryIsClampedNotSilenced) {
    // Silence is a worse answer than the highest note the hardware can make.
    ToneGenerator tone(16000);
    tone.start(20000, 100);
    NOTRIX_CHECK(tone.playing());
}
