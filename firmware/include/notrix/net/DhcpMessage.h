...
///
/// **Why NOTRIX has to speak this at all.** There is no DHCP client on the
...
/// every NOTRIX session so far has had an address: each one began by stopping
/// a program that already had one. Nothing renews it, so a NOTRIX device left
...
struct Request {
    ...
    /// list, and a row saying "notrix" beats a row saying nothing.
