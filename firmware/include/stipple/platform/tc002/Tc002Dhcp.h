...
/// not know about.
///
/// The protocol, the retry timing and the framing live in stipple::net, above
...
///
/// **STIPPLE has to do this itself.** There is no DHCP client on the TC002:
...
/// STIPPLE session so far has had an address - each one began by stopping a
/// program that already had one. Nothing renews it, so a STIPPLE device left
