# 0019 — NOTRIX speaks DHCP, because nothing else here can
...
**NOTRIX has never held a lease.** Every session has run on an address the
vendor application obtained *before* NOTRIX started: each one begins with
...
lease this device is issued is **86400 seconds**. A NOTRIX device left alone
loses its network after a day, and nobody has seen it because no unit has run
that long without a restart.

So this is not a provisioning feature. It is a thing NOTRIX needs in order to
...
`notrix::net::dhcp` holds the message format, the state machine and the
...
need it. NOTRIX already runs as root on this device, so nothing changes, but
