# 0021 — STIPPLE is the startup library, and it lives in /data
...
it. Blueprint §7.1 said STIPPLE becomes `/res/lib/libzkgui.so`, the 7.5 MB
...
with `/proc/<pid>/maps` showing `/tmp/libstipple.so` mapped and
...
```json
"startupLibPath": "/data/stipple/libstipple.so"
```
...
writable persistent filesystem. Putting STIPPLE there means the `res`
partition is written exactly once, to change one JSON field, and every
STIPPLE release after that is a file copy over the network. No flashing, no
...
device.** The claim was that a missing `/data/stipple/libstipple.so` leaves a
...
**§53 survives.** STIPPLE does not implement a vendor interface; it is
...
**Put STIPPLE in `/res` beside the vendor app.** Then every STIPPLE update is
...
[ADR 0008](0008-installer-helper.md). The decision here still stands - STIPPLE
...
Today `startupLibPath` points straight at `/data/stipple/libstipple.so`. If
...
```
startupLibPath  ->  /res/lib/libstippleboot.so
                      constructor: dlopen("/data/stipple/libstipple.so")
                        loaded   -> STIPPLE takes the process, never returns
...
So a missing or broken STIPPLE degrades to **the stock clock**, not to a
...
**STIPPLE present.** The shim loaded it and STIPPLE took the process.
`/proc/<pid>/maps` showed all three libraries: the shim, STIPPLE, and
...
STIPPLE will not load.

**STIPPLE absent.** The point of the whole exercise:

```
mapped:   /tmp/libstippleboot.so, /res/lib/libzkgui.so   (no STIPPLE)
log:      no stipple, starting the stock clock:
          /data/stipple/libstipple.so: cannot open shared object file
...
**And STIPPLE as a library holds its own lease**, which had never been
